...
internal sealed class ModelEvaluationModifier : IHtmlModifier
{
    ...
    public void Modify(IDocument document)
    {
        ...
        foreach (IElement p in paragraphs)
        {
            ...
            List<LocalModelEvalution>? models = null;
            try
            {
                models = JsonSerializer.Deserialize<List<LocalModelEvalution>>(json, _serializer_options);
            }
            catch (JsonException)
            {
            ...
            if (gptIndex > 0)
            {
                LocalModelEvalution gptItem = models[gptIndex];
            ...
            foreach (LocalModelEvalution model in models)
            {
                if (model is null ||
