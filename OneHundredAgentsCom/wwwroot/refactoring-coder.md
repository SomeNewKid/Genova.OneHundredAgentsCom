...
Refactoring Coder is a small proof of concept built to take first steps with Anthropic's Python SDK. The idea is simple enough to keep the moving parts visible: point an agent at an existing bit of code, ask it to work out the current behaviour, write unit tests for that behaviour, and only then refactor the code. That is the interesting part. The agent is not just chatting about code. It is using local tools to read files, save changes, and run the project checks.
...
Language: Python
Framework: [Anthropic Python SDK](https://platform.claude.com/docs/en/api/sdks/python)
