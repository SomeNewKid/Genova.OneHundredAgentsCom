...
internal static class PageRenderer
{
        ...
        IRequestContext requestContext, string query, string? bodyClass = null, string? heroImageName = null)
    {
        ...
        string heroImage = string.IsNullOrEmpty(heroImageName) ? "" :
            ThemeSpecificImages ?
              $"""
              <figure class="hero">
                <picture data-theme-swap>
                  <source srcset="/-/images/heroes/{heroImageName}-dark.jpg{buster}" media="(prefers-color-scheme: dark)">
                  <img src="/-/images/heroes/{heroImageName}-light.png{buster}" alt="" role="presentation" width="988" height="329"> 
                </picture>
              </figure>
              """
              :
...
""");
        ...
        if (ThemeSpecificImages && !string.IsNullOrEmpty(heroImage))
        {
            html.AppendLine("""
  <script nonce="{nonce}">
    (function(){var d=document;
      var osIsDark=false;try{osIsDark=!!(window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches);}catch(e){}
      var storedTheme=null;try{storedTheme=localStorage.getItem('pow-theme');}catch(e){}
      if(storedTheme!=='dark'&&storedTheme!=='light'){return;}
      var mismatchDark=!osIsDark&&storedTheme==='dark';
      var mismatchLight=osIsDark&&storedTheme==='light';
      var isMismatch=mismatchDark||mismatchLight;
      if(!isMismatch){return;}
      var html=d.documentElement;
      html.classList.add('theme-prehide');
      var style=d.createElement('style');
      var nonce=(d.currentScript&&d.currentScript.nonce)||'';
      if(nonce)style.setAttribute('nonce',nonce);
      style.textContent='html.theme-prehide picture[data-theme-swap] > img{opacity:0;}';
      d.head.appendChild(style);
      function pickDarkURL(pic){var src=pic&&pic.querySelector('source[media*="(prefers-color-scheme: dark)"]');if(!src)return null;var set=(src.getAttribute('srcset')||'').trim();if(!set)return null;var first=set.split(",")[0].trim();return first.split(/\s+/)[0]||null;}
      function pickLightURL(pic){var img=pic&&pic.querySelector('img');if(!img)return null;var u=(img.getAttribute('src')||'').trim();return u||null;}
      function onReady(){var heroPic=d.querySelector('picture[data-theme-swap]');if(heroPic){var preloadURL=mismatchDark?pickDarkURL(heroPic):pickLightURL(heroPic);if(preloadURL){var link=d.createElement('link');link.rel='preload';link.as='image';link.href=preloadURL;d.head.appendChild(link);}}setTimeout(function(){html.classList.remove('theme-prehide');},2000);}
      if(d.readyState==='loading'){d.addEventListener('DOMContentLoaded',onReady,{once:true});}else{onReady();}
    })();
  </script>
""");
        }
